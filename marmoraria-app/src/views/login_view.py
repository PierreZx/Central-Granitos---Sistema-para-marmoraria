...
def LoginView(page: ft.Page):
    # Cores fixas dentro da view para garantir carregamento na Web
    VINHO = "#722F37"
    BRANCO = "#FFFFFF"
    ...
    def realizar_login(e):
        if campo_usuario.value == "admin" and campo_senha.value == "123":
            ...
            page.overlay.append(snack) # Melhor que page.snack_bar na web
            snack.open = True
            page.update()

    # UI simples para testar a renderização
    logo = ft.Text("CENTRAL GRANITOS", size=32, weight="bold", color=VINHO)
    
    campo_usuario = ft.TextField(label="Utilizador", width=300, border_radius=10)
