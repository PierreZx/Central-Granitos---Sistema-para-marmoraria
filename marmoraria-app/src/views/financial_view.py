...
def FinancialView(page: ft.Page):
    ...
    def mostrar_confirmacao(titulo, mensagem, ao_confirmar, tipo="danger"):
        ...
        dlg_confirm = ft.AlertDialog(
            ...
            ], horizontal_alignment=ft.CrossAxisAlignment.CENTER, width=350),
            actions=[
                ft.TextButton(
                    "Cancelar", 
                    style=ft.ButtonStyle(color={"": ft.colors.GREY_600}),
                    on_click=lambda e: fechar_dlg(dlg_confirm)
                ),
                ft.ElevatedButton(
                    "Confirmar", 
                    ...
                    on_click=lambda e: ao_confirmar()
                )
            ],
    ...
    def carregar_dados():
        ...
        card_saldo = ft.Container(
            bgcolor=COLOR_WHITE,
            padding=padding_cards,
            border_radius=BORDER_RADIUS_LG,
            shadow=SHADOW_MD,
            content=ft.Row([
                ft.Container(
                    width=60 if eh_mobile else 80,
                    height=60 if eh_mobile else 80,
                    border_radius=30 if eh_mobile else 40,
                    bgcolor=f"{COLOR_PRIMARY}10", 
                    content=ft.Icon(
                        ft.icons.ACCOUNT_BALANCE_WALLET, 
                        size=30 if eh_mobile else 40, 
                        color=COLOR_PRIMARY
                    ),
                    alignment=ft.alignment.center
                ),
        ...
        if not lista_dividas:
            ...
            for div in lista_dividas:
                ...
                coluna_dividas.controls.append(ft.Container(
                    bgcolor=COLOR_WHITE, padding=15, border_radius=BORDER_RADIUS_MD, shadow=SHADOW_MD,
        ...
        if not lista_receber: coluna_receber.controls.append(ft.Text("Nada a receber.", color="grey"))
            ...
            for orc in lista_receber:
                ...
                coluna_receber.controls.append(ft.Container(
                    bgcolor=COLOR_WHITE, padding=15, border_radius=BORDER_RADIUS_MD, shadow=SHADOW_MD, content=c_rec
                ))
    ...
    def abrir_add_divida(e, divida_existente=None):
        ...
        # Conteúdo do Dialog (Corrigido para não quebrar)
        conteudo_dlg = ft.Column([
            ...
            txt_qtd_parcelas, 
            txt_desc_divida
        ], tight=True, width=300 if eh_mobile else 400)
    ...
    dlg_add_divida = ft.AlertDialog(title=ft.Text("Conta"), actions=[ft.TextButton("Cancelar", on_click=lambda e: page.close_dialog()), btn_salvar_divida])
    
    # CORREÇÃO DA SINTAXE AQUI (Removido barras invertidas)
    dlg_add_manual = ft.AlertDialog(
        title=ft.Text("Lançamento", weight="bold"), 
        content=ft.Column([dd_tipo_mov, txt_valor_mov, txt_desc_mov], tight=True, width=300),
