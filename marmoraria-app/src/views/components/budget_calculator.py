...
class BudgetCalculator(ft.UserControl):
    ...
    def build(self):
        ...
        # Recortes (CORRIGIDO AQUI: Definindo com os nomes corretos)
        self.chk_cuba = ft.Checkbox(label="Cuba", value=False)
        self.txt_cuba_pos = ft.TextField(label="Dist. Esq (m)", value="0.50", width=100, height=40, text_size=12, disabled=True) # Era txt_pos_cuba
        self.txt_cuba_larg = ft.TextField(label="Largura", value="0.50", width=80, height=40, text_size=12, disabled=True)
        self.txt_cuba_prof = ft.TextField(label="Profund.", value="0.40", width=80, height=40, text_size=12, disabled=True)

        self.chk_cook = ft.Checkbox(label="Cooktop", value=False)
        self.txt_cook_pos = ft.TextField(label="Dist. Esq (m)", value="1.50", width=100, height=40, text_size=12, disabled=True) # Era txt_pos_cook
        self.txt_cook_larg = ft.TextField(label="Largura", value="0.60", width=80, height=40, text_size=12, disabled=True)
        self.txt_cook_prof = ft.TextField(label="Profund.", value="0.45", width=80, height=40, text_size=12, disabled=True)

        # --- NOVA ABA: INSTRUÇÕES DE PRODUÇÃO ---
        checklist_padrao = """🔹 Acabamento Borda: Reto / 45º / Boleado
🔹 Tipo de Cuba: Embutir / Sobrepor
🔹 Furos de Torneira: (Qtd e Posição)
🔹 Reforço: Sim / Não (Onde?)
🔹 Observações: """
        
        self.txt_instrucoes = ft.TextField(
            label="Instruções Técnicas para o Serralheiro",
            value=checklist_padrao,
            multiline=True,
            min_lines=8,
            max_lines=12,
            text_size=14,
        ...
        if self.item_para_editar:
            ...
            self.txt_instrucoes.value = cfg.get('instrucoes_producao', checklist_padrao)
        ...
        tabs = ft.Tabs(selected_index=0, animation_duration=300, tabs=[
            ...
            ft.Tab(text="4. Recortes", content=ft.Container(padding=15, content=ft.Column([
                ft.Row([self.chk_cuba, self.txt_cuba_pos], spacing=10),
                ft.Row([self.txt_cuba_larg, self.txt_cuba_prof], spacing=10),
                ft.Divider(),
                ft.Row([self.chk_cook, self.txt_cook_pos], spacing=10),
                ft.Row([self.txt_cook_larg, self.txt_cook_prof], spacing=10)
            ]))),
            ft.Tab(text="5. Produção", icon=ft.icons.ASSIGNMENT, content=ft.Container(padding=15, content=ft.Column([
                ft.Text("Instruções para o chão de fábrica:", weight="bold", color=COLOR_PRIMARY),
                self.txt_instrucoes
            ], expand=True)))
        ], expand=True)
