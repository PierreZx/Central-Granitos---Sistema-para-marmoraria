...
class BudgetCalculator(ft.UserControl):
    ...
    def build(self):
        ...
        # --- FUNÇÃO HELPER PARA VALIDAÇÃO DE NÚMEROS ---
        def on_num_change(e):
        ...
        # Teclado ajustado para NUMBER_WITH_OPTIONS para liberar o ponto/vírgula
        self.txt_larg = ft.TextField(label="Comprimento (m)", value="1.00", expand=True, keyboard_type=ft.KeyboardType.NUMBER_WITH_OPTIONS, on_change=on_num_change)
        self.txt_prof = ft.TextField(label="Profundidade (m)", value="0.60", expand=True, keyboard_type=ft.KeyboardType.NUMBER_WITH_OPTIONS, on_change=on_num_change)
        self.txt_acab = ft.TextField(label="Mão de Obra (R$/ML)", value="130.00", expand=True, keyboard_type=ft.KeyboardType.NUMBER_WITH_OPTIONS, on_change=on_num_change)

        def campo_alt(val): 
            return ft.TextField(label="Alt (m)", value=val, width=75, height=40, text_size=12, keyboard_type=ft.KeyboardType.NUMBER_WITH_OPTIONS, on_change=on_num_change)
        ...
        self.bojo_w = ft.TextField(label="Larg (m)", value="0.50", width=75, height=40, text_size=11, keyboard_type=ft.KeyboardType.NUMBER_WITH_OPTIONS, on_change=on_num_change)
        self.bojo_h = ft.TextField(label="Prof (m)", value="0.40", width=75, height=40, text_size=11, keyboard_type=ft.KeyboardType.NUMBER_WITH_OPTIONS, on_change=on_num_change)
        self.bojo_x = ft.TextField(label="Eixo (m)", value="0.50", width=75, height=40, text_size=11, keyboard_type=ft.KeyboardType.NUMBER_WITH_OPTIONS, on_change=on_num_change)

        self.sw_cook = ft.Switch(label="Furo Cooktop", value=False, on_change=self.calcular)
        self.cook_w = ft.TextField(label="Larg (m)", value="0.55", width=75, height=40, text_size=11, keyboard_type=ft.KeyboardType.NUMBER_WITH_OPTIONS, on_change=on_num_change)
        self.cook_h = ft.TextField(label="Prof (m)", value="0.45", width=75, height=40, text_size=11, keyboard_type=ft.KeyboardType.NUMBER_WITH_OPTIONS, on_change=on_num_change)
        self.cook_x = ft.TextField(label="Eixo (m)", value="1.50", width=75, height=40, text_size=11, keyboard_type=ft.KeyboardType.NUMBER_WITH_OPTIONS, on_change=on_num_change)
    ...
    def calcular(self, e=None):
        try:
            ...
            l, p = float(self.txt_larg.value or 0), float(self.txt_prof.value or 0)
    ...
    def desenhar(self, w, h):
        ...
        # Furos (O segredo do Cooktop estar aqui)
        def draw_furo(sw, fx, fw, fh):
            if sw.value:
                pos_x = sx + (float(fx.value or 0) * scale)
                fw_px = float(fw.value or 0) * scale
                fh_px = float(fh.value or 0) * scale
                # Desenha o furo técnico proporcional ao tamanho real inserido
                self.canvas.shapes.append(cv.Rect(pos_x - fw_px/2, sy + (h_px-fh_px)/2, fw_px, fh_px, border_radius=3, paint=ft.Paint(style="stroke", color="blue", stroke_width=1.5)))
